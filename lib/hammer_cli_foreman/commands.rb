module HammerCLIForeman
  ...
  def self.resource_config
    ...
    config[:username] = HammerCLI::Settings.get(:username) || ENV['FOREMAN_USERNAME'] || HammerCLI::Settings.get(:foreman, :username)
    config[:password] = HammerCLI::Settings.get(:password) || ENV['FOREMAN_PASSWORD'] || HammerCLI::Settings.get(:foreman, :password)
    config
  end

  def self.connection_options
    config = {}
    config[:service] = 'Foreman'
    config
  end
